...
CREATE TABLE `nebula_publish_host` (
  ...
  `publish_project` varchar(50) DEFAULT NULL COMMENT '发布工程名',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_module_id_project` (`publish_module_id`,`publish_project`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
...
CREATE TABLE `nebula_publish_module` (
  ...
  `publish_event_id` int(11) DEFAULT NULL COMMENT '发布事件id',
  `publish_module` varchar(50) DEFAULT NULL COMMENT '发布模块',
